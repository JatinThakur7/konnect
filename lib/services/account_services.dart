...
class AccountDocument {
  final DocumentReference<CompanyModal> reference;

  final CollectionReference<ItemModal> itemReference;
  ...
  AccountDocument(QueryDocumentSnapshot<CompanyModal> query)
    ...
    print('Account Document Created ${reference.id}');
  }

  /* ********************************************************** */

  Future<List<ItemModal>> getItems() async {
    var snapshot = await itemReference.get();
    print('${itemReference.id} has ${snapshot.size} document');
